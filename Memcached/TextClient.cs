...
#region Memcached protocol interface
    ...
    {
        this.SendCommand (PROTOCOL_ADD, key, data, expiration);
    }

    public override void Append (string key, object data)
    {
        this.SendCommand (PROTOCOL_APPEND, key, data);
    }

    public override void Cas (string key, object data, TimeSpan expiration, ulong casUnique)
    {
        this.SendCommand (PROTOCOL_CAS, key, data, expiration, casUnique);
    }

    public override void Decrement (string key, int amount)
    {
        this.SendCommand (PROTOCOL_DECREMENT, key, amount);
    }

    public override void Delete (string key)
    {
        this.SendCommand (PROTOCOL_DELETE, key);
    }

    public override void FlushAll (TimeSpan delay)
    {
        this.SendCommand (PROTOCOL_FLUSHALL, delay);
    }

    public override KeyValuePair <string, object> Get (string key)
    {
        KeyValuePair <string, object> [] kvp = this.Gets (key);
    ...
    {
        this.SendCommand (PROTOCOL_INCREMENT, key, amount);
    }

    public override void Prepend (string key, object data)
    {
        this.SendCommand (PROTOCOL_PREPEND, key, data);
    }

    public override void Replace (string key, object data, TimeSpan expiration)
    {
        this.SendCommand (PROTOCOL_REPLACE, key, data, expiration);
    }

    public override void Set (string key, object data, TimeSpan expiration)
    {
        this.SendCommand (PROTOCOL_SET, key, data, expiration);
    }
...
#region Support methods
    ...
    private void SendCommand (string cmd, string key, object data, TimeSpan expiration, ulong casUnique)
    {
        ...
        byte [] buf = this._encoding.GetBytes (data.ToString ());
        string  s   = this._encoding.GetString (buf, 0, buf.Length);
    ...
    {
        string s = this._encoding.GetString (res, 0, res.Length);
    ...
    {
        byte [] data = this._encoding.GetBytes (sData);
    ...
    {
        // VALUE key2 10 9 2\r\n111222333\r\nEND\r\n
        string []                            sInput = this._encoding.GetString (input, 0, input.Length).Split (new string [] {"\r\n"}, StringSplitOptions.None);
    ...
    private byte [] GetResponse ()
    {
        ...
        int          cnt = this.stream.Read (res, 0, 1024);
        ...
        while (cnt > 0)
        {
            ...
            cnt = this.stream.Read (res, 0, 1024);
        }

        byte [] res2 = ms.ToArray ();
        this.ValidateErrorResponse (res2);
        return res2;
    }

...
public struct MySqlGeometry : IMySqlValue
{
    ...
    /// </summary>
    public double? XCoordinate => this._xValue;

    /// <summary>
    /// Gets the y coordinate.
    /// </summary>
    public double? YCoordinate => this._yValue;

    /// <summary>
    /// Gets the SRID value.
    /// </summary>
    public int? SRID => this._srid;
    ...
    internal MySqlGeometry (MySqlDbType type, double xValue, double yValue, int srid)
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            this.Value [i + 9] =   (byte) (xVal & 0xff);
            xVal               >>= 8;
        }

        for (int i = 0; i < 8; i++)
        {
            this.Value [i + 17] =   (byte) (yVal & 0xff);
            yVal                >>= 8;
        }
    }
    ...
    public MySqlGeometry (MySqlDbType type, byte [] val)
    {
        ...
        int xIndex = val.Length == GEOMETRY_LENGTH ? 9 : 5;
        int yIndex = val.Length == GEOMETRY_LENGTH ? 17 : 13;

        this.Value   = buffValue;
        this._xValue = val.Length >= xIndex + 8 ? BitConverter.ToDouble (val, xIndex) : 0;
        this._yValue = val.Length >= yIndex + 8 ? BitConverter.ToDouble (val, yIndex) : 0;
...
#region IMySqlValue Members
    ...
    IMySqlValue IMySqlValue.ReadValue (MySqlPacket packet, long length, bool nullVal)
    {
        ...
        if (nullVal)
        {
            g = new MySqlGeometry (this._type, true);
        }
        else
        {
            if (length == -1)
            ...
            g = new MySqlGeometry (this._type, newBuff);
        }
    ...
    public override string ToString ()
    {
        if (!this.IsNull)
            return this._srid != 0
                ? string.Format (CultureInfo.InvariantCulture.NumberFormat, "SRID={2};POINT({0} {1})", this._xValue, this._yValue, this._srid)
                : string.Format (CultureInfo.InvariantCulture.NumberFormat, "POINT({0} {1})",          this._xValue, this._yValue);
        ...
        try
        {
            ...
            if (arrayResult.Length > 1 || strResult != string.Empty)
            {
                ...
                if (arrayResult.Length >= 1)
                    int.TryParse (arrayResult [0].Replace ("SRID=", ""), out sridValue);
            }
    ...
    {
        if (!this.IsNull)
            return string.Format (CultureInfo.InvariantCulture.NumberFormat, "POINT({0} {1})", this._xValue, this._yValue);

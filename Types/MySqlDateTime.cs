...
public struct MySqlDateTime : IMySqlValue, IComparable, IConvertible
{
    ...
    public MySqlDateTime (MySqlDateTime mdt)
    {
        ...
        this._microsecond   = 0;
        this._millisecond   = 0;
        this._type          = MySqlDbType.DateTime;
        this.IsNull         = false;
    ...
    (
        MySqlDbType type,   int year, int month, int day, int hour, int minute,
        ...
        : this (type, 0, 0, 0, 0, 0, 0, 0)
    {
        ...
        this.Microsecond = (int) (val.Ticks % 10000000) / 10;
    }
...
#region Properties
    ...
    /// </summary>
    public bool IsValidDateTime => this.Year != 0 && this.Month != 0 && this.Day != 0;
...
#region IMySqlValue Members
    ...
    object IMySqlValue.Value => this.GetDateTime ();

    /// <summary>
    /// Retrieves the value of this <see cref="MySqlDateTime"/> as a DateTime object.
    /// </summary>
    public DateTime Value => this.GetDateTime ();
    ...
    private void SerializeText (MySqlPacket packet, MySqlDateTime value)
    {
        ...
        if (this._type != MySqlDbType.Date)
    ...
    void IMySqlValue.WriteValue (MySqlPacket packet, bool binary, object value, int length)
    {
        ...
        if (value is DateTime)
            dtValue = new MySqlDateTime (this._type, (DateTime) value);
        ...
        if (!binary)
        {
            this.SerializeText (packet, dtValue);
            return;
        }
    ...
    private MySqlDateTime ParseMySql (string s)
    {
        ...
        return new MySqlDateTime (this._type, year, month, day, hour, minute, second, microsecond);
    }

    IMySqlValue IMySqlValue.ReadValue (MySqlPacket packet, long length, bool nullVal)
    {
        if (nullVal)
            return new MySqlDateTime (this._type, true);

        if (length >= 0)
        {
            string value = packet.ReadString (length);
            return this.ParseMySql (value);
        }
    ...
    public DateTime GetDateTime ()
    {
        ...
        if (this._type == MySqlDbType.Timestamp)
        {
        ...
        return new DateTime (this.Year, this.Month, this.Day, this.Hour, this.Minute, this.Second, kind).AddTicks (this._microsecond * 10);
    }
    ...
    public override string ToString ()
    {
        if (this.IsValidDateTime)
        {
            DateTime d = new DateTime (this.Year, this.Month, this.Day, this.Hour, this.Minute, this.Second).AddTicks (this._microsecond * 10);
            return this._type == MySqlDbType.Date ? d.ToString ("d", CultureInfo.InvariantCulture) : d.ToString (CultureInfo.InvariantCulture);
        }

        string dateString = FormatDateCustom (CultureInfo.CurrentUICulture.DateTimeFormat.ShortDatePattern, this.Month, this.Day, this.Year);

        if (this._type == MySqlDbType.Date)
            return dateString;

        DateTime dt = new DateTime (1, 2, 3, this.Hour, this.Minute, this.Second).AddTicks (this._microsecond * 10);
    ...
    internal static void SetDSInfo (MySqlSchemaCollection sc)
    {
        ...
        MySqlDbType [] dbtype = new MySqlDbType [] {MySqlDbType.Date, MySqlDbType.DateTime, MySqlDbType.Timestamp};
...
#region IComparable Members
    ...
    int IComparable.CompareTo (object obj)
    {
        ...
        if (this.Second < otherDate.Second)
            return -1;
        else if (this.Second > otherDate.Second)
            return 1;

        if (this.Microsecond < otherDate.Microsecond)
            return -1;
        else if (this.Microsecond > otherDate.Microsecond)
            return 1;
...
#region IConvertible Members
    ...
    {
        return this.GetDateTime ().ToString (provider);
    }
    ...
    {
        return conversionType == typeof (DateTime) ? (object) this.GetDateTime ()

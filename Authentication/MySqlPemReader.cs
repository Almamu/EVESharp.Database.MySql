...
public class MySqlPemReader
{
        ...
        using (MemoryStream memoryStream = new MemoryStream (key))
        {
            using (BinaryReader reader = new BinaryReader (memoryStream))
            {
                try
                {
                    ...
                    byte [] sequence = reader.ReadBytes (15);
    ...
    private static byte [] DecodeOpenSslKey (byte [] rawPublicKey)
    {
        ...
        byte [] footerSequence =
        {
            0x2D, 0x2D, 0x2D, 0x2D, 0x2D, 0x45, 0x4E, 0x44, 0x20, 0x50, 0x55, 0x42, 0x4C, 0x49, 0x43, 0x20, 0x4B, 0x45, 0x59, 0x2D, 0x2D, 0x2D, 0x2D, 0x2D
        };

        if (!StartsWith (rawPublicKey, headerSequence) || !EndsWith (rawPublicKey, footerSequence))
            return null;
    ...
    private static bool EndsWith (byte [] array, byte [] containedArray)
    {
        ...
        {
            if (j == containedArray.Length)
                break;

            if (array [i] != containedArray [containedArray.Length - j - 1])
                return false;
        }

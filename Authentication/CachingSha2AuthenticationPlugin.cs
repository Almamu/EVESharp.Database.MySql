...
internal class CachingSha2AuthenticationPlugin : Sha256AuthenticationPlugin
{
    ...
    protected override byte [] MoreData (byte [] data)
    {
        ...
        if (data == null)
        {
            byte [] scramble = this.GetPassword () as byte [];
        ...
        {
            return this.GeneratePassword () as byte [];
        }
    }
    ...
    protected byte [] GeneratePassword ()
    {
        ...
        else
        {
            ...
            if (this.rawPubkey != null && this.rawPubkey [0] == 4)
            {
    ...
    public override object GetPassword ()
    {
        ...
        byte [] firstHash  = sha.ComputeHash (Encoding.Default.GetBytes (this.GetMFAPassword ()));
        byte [] secondHash = sha.ComputeHash (firstHash);

        byte [] input = new byte[this.AuthenticationData.Length + secondHash.Length];
        Array.Copy (secondHash,              0, input, 0,                 secondHash.Length);
        Array.Copy (this.AuthenticationData, 0, input, secondHash.Length, this.AuthenticationData.Length);

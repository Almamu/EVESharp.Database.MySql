...
internal class SchemaProvider
{
    ...
    public virtual MySqlSchemaCollection GetSchema (string collection, string [] restrictions)
    {
        ...
        MySqlSchemaCollection c = this.GetSchemaInternal (collection, restrictions);
    ...
    {
        Regex regex       = null;
        int   caseSetting = int.Parse (this.connection.driver.Property ("lower_case_table_names"), CultureInfo.InvariantCulture);
        ...
        MySqlSchemaCollection c = this.QueryCollection ("Databases", sql);
    ...
    public virtual MySqlSchemaCollection GetTables (string [] restrictions)
    {
        ...
        MySqlSchemaCollection databases = this.GetDatabases (dbRestriction);

        if (restrictions != null)
            Array.Copy (
                restrictions, dbRestriction,
        ...
        foreach (MySqlSchemaRow row in databases.Rows)
        {
            ...
            this.FindTables (c, dbRestriction);
        }
    ...
    public virtual MySqlSchemaCollection GetColumns (string [] restrictions)
    {
        ...
        foreach (MySqlSchemaRow row in tables.Rows)
            this.LoadTableColumns (
                c, row ["TABLE_SCHEMA"].ToString (),
                row ["TABLE_NAME"].ToString (), columnName
            );

        this.QuoteDefaultValues (c);
        return c;
    }

    private void LoadTableColumns
    (
        MySqlSchemaCollection schemaCollection, string schema,
        string                tableName,        string columnRestriction
    )
    {
        string sql = string.Format (
            "SHOW FULL COLUMNS FROM `{0}`.`{1}`",
            schema, tableName
        );
    ...
    public virtual MySqlSchemaCollection GetIndexes (string [] restrictions)
    {
        ...
        MySqlSchemaCollection tables = this.GetTables (tableRestrictions);
        ...
        foreach (MySqlSchemaRow table in tables.Rows)
        {
            ...
            MySqlSchemaCollection indexes = this.QueryCollection ("indexes", sql);

            foreach (MySqlSchemaRow index in indexes.Rows)
            {
                if (1 != (this.connection.driver.Version.isAtLeast (8, 0, 1) ? (uint) index ["SEQ_IN_INDEX"] : (long) index ["SEQ_IN_INDEX"]))
                    continue;
                ...
                row ["UNIQUE"] = this.connection.driver.Version.isAtLeast (8, 0, 1)
    ...
    public virtual MySqlSchemaCollection GetForeignKeys (string [] restrictions)
    {
        ...
        foreach (MySqlSchemaRow table in tables.Rows)
            this.GetForeignKeysOnTable (dt, table, keyName, false);
    ...
    public virtual MySqlSchemaCollection GetForeignKeyColumns (string [] restrictions)
    {
        ...
        // parse it's foreign keys
        foreach (MySqlSchemaRow table in tables.Rows)
            this.GetForeignKeysOnTable (dt, table, keyName, true);
...
#region Foreign Key routines
    ...
    (
        MySqlSchemaCollection fkTable,    MySqlSchemaRow tableToParse,
    ...
    (
        MySqlSchemaCollection fkTable,   MySqlSchemaRow table,
    ...
    public virtual MySqlSchemaCollection GetUsers (string [] restrictions)
    {
        ...
        MySqlSchemaCollection c = this.QueryCollection ("Users", sb.ToString ());
    ...
    public virtual MySqlSchemaCollection GetProcedures (string [] restrictions)
    {
        ...
        if (this.connection.driver.Version.isAtLeast (8, 0, 1))
        {
            ...
            using (MySqlDataReader reader = cmd.ExecuteReader ())
            {
                while (reader.Read ())
                {
                    ...
                    row ["DTD_IDENTIFIER"] = StringUtility.ToLowerInvariant (routineType) == "function"
                        ? (object) reader.GetString ("DTD_IDENTIFIER")
                        : DBNull.Value;
    ...
    private MySqlSchemaCollection GetDataSourceInformation ()
    {
        ...
        DBVersion v   = this.connection.driver.Version;
    ...
    protected virtual MySqlSchemaCollection GetSchemaInternal (string collection, string [] restrictions)
    {
        ...
        {
            // common collections
            case "METADATACOLLECTIONS":   return this.GetCollections ();
            case "DATASOURCEINFORMATION": return this.GetDataSourceInformation ();
            case "DATATYPES":             return GetDataTypes ();
            case "RESTRICTIONS":          return this.GetRestrictions ();
            case "RESERVEDWORDS":         return GetReservedWords ();

            // collections specific to our provider
            case "USERS":     return this.GetUsers (restrictions);
            case "DATABASES": return this.GetDatabases (restrictions);
            case "UDF":       return this.GetUDF (restrictions);
        }
        ...
        if (this.connection != null && this.connection.Database != null && this.connection.Database.Length > 0 &&
        ...
        switch (collection)
        {
            case "TABLES":              return this.GetTables (restrictions);
            case "COLUMNS":             return this.GetColumns (restrictions);
            case "INDEXES":             return this.GetIndexes (restrictions);
            case "INDEXCOLUMNS":        return this.GetIndexColumns (restrictions);
            case "FOREIGN KEYS":        return this.GetForeignKeys (restrictions);
            case "FOREIGN KEY COLUMNS": return this.GetForeignKeyColumns (restrictions);
        }

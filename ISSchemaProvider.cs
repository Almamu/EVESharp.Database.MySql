...
internal class ISSchemaProvider : SchemaProvider
{
    ...
    public override MySqlSchemaCollection GetDatabases (string [] restrictions)
    {
        ...
        keys [0] = "SCHEMA_NAME";
        MySqlSchemaCollection dt = this.Query ("SCHEMATA", "", keys, restrictions);
    ...
    public override MySqlSchemaCollection GetTables (string [] restrictions)
    {
        ...
        keys [2] = "TABLE_NAME";
        keys [3] = "TABLE_TYPE";
        MySqlSchemaCollection dt = this.Query ("TABLES", "TABLE_TYPE != 'VIEW'", keys, restrictions);
        dt.Name = "Tables";
        return dt;
    }
    ...
    public override MySqlSchemaCollection GetColumns (string [] restrictions)
    {
        ...
        keys [2] = "TABLE_NAME";
        keys [3] = "COLUMN_NAME";
        MySqlSchemaCollection dt = this.Query ("COLUMNS", null, keys, restrictions);
        dt.RemoveColumn ("CHARACTER_OCTET_LENGTH");
        dt.Name = "Columns";
        this.QuoteDefaultValues (dt);
        return dt;
    }
    ...
    private MySqlSchemaCollection GetViews (string [] restrictions)
    {
        ...
        keys [1] = "TABLE_SCHEMA";
        keys [2] = "TABLE_NAME";
        MySqlSchemaCollection dt = this.Query ("VIEWS", null, keys, restrictions);
        dt.Name = "Views";
        return dt;
    }
    ...
    private MySqlSchemaCollection GetViewColumns (string [] restrictions)
    {
        ...
        MySqlSchemaCollection dt = this.GetTable (sql.ToString ());
        dt.Name             = "ViewColumns";
    ...
    private MySqlSchemaCollection GetTriggers (string [] restrictions)
    {
        ...
        keys [3] = "TRIGGER_NAME";
        MySqlSchemaCollection dt = this.Query ("TRIGGERS", null, keys, restrictions);
        dt.Name = "Triggers";
        return dt;
    }
    ...
    public override MySqlSchemaCollection GetProcedures (string [] restrictions)
    {
        ...
        MySqlSchemaCollection dt = this.Query ("ROUTINES", null, keys, restrictions);
        dt.Name = "Procedures";
        return dt;
    }
    ...
    {
        MySqlSchemaCollection dt = this.GetProcedures (restrictions);
    ...
    private string GetProcedureParameterLine (MySqlSchemaRow isRow)
    {
        ...
        sql = string.Format (
            sql, isRow ["ROUTINE_TYPE"], isRow ["ROUTINE_SCHEMA"],
            isRow ["ROUTINE_NAME"]
        );
    ...
    private MySqlSchemaCollection GetParametersForRoutineFromIS (string [] restrictions)
    {
        ...
        MySqlSchemaCollection coll = this.QueryCollection ("parameters", sql.ToString ());
    ...
    private MySqlSchemaCollection GetParametersFromIS (string [] restrictions, MySqlSchemaCollection routines)
    {
        ...
        if (routines == null || routines.Rows.Count == 0)
        {
            if (restrictions == null)
                parms = this.QueryCollection ("parameters", "SELECT * FROM INFORMATION_SCHEMA.PARAMETERS WHERE 1=2");
            else
                parms = this.GetParametersForRoutineFromIS (restrictions);
        }
        else
        {
    ...
    internal MySqlSchemaCollection CreateParametersTable ()
    {
        ...
        dt.AddColumn ("CHARACTER_MAXIMUM_LENGTH", typeof (int));
        dt.AddColumn ("CHARACTER_OCTET_LENGTH",   typeof (int));
        dt.AddColumn ("NUMERIC_PRECISION",        typeof (byte));
        dt.AddColumn ("NUMERIC_SCALE",            typeof (int));
    ...
    /// </summary>
    public virtual MySqlSchemaCollection GetProcedureParameters
    (
        string []             restrictions,
        MySqlSchemaCollection routines
    )
    {
        bool is55 = this.connection.driver.Version.isAtLeast (5, 5, 3);

        try
        {
            // we want to avoid using IS if  we can as it is painfully slow
            MySqlSchemaCollection dt = this.CreateParametersTable ();
            this.GetParametersFromShowCreate (dt, restrictions, routines);
            return dt;
        }
        catch (Exception)
        {
    ...
    protected override MySqlSchemaCollection GetSchemaInternal (string collection, string [] restrictions)
    {
        ...
        switch (collection)
        {
            case "VIEWS":                      return this.GetViews (restrictions);
            case "PROCEDURES":                 return this.GetProcedures (restrictions);
            case "PROCEDURES WITH PARAMETERS": return this.GetProceduresWithParameters (restrictions);
            case "PROCEDURE PARAMETERS":       return this.GetProcedureParameters (restrictions, null);
            case "TRIGGERS":                   return this.GetTriggers (restrictions);
            case "VIEWCOLUMNS":                return this.GetViewColumns (restrictions);
        }
    ...
    private MySqlSchemaCollection Query
    (
        string    tableName, string    initialWhere,
    ...
    )
    {
        ...
        return this.GetTable (query.ToString ());
    }
...
#region Procedures Support Routines
    ...
    )
    {
        ...
        foreach (MySqlSchemaRow routine in routines.Rows)
        {
            string showCreateSql = string.Format (
                "SHOW CREATE {0} `{1}`.`{2}`",
            ...
            try
            {
                ...
                using (MySqlDataReader reader = cmd.ExecuteReader ())
                {
                    ...
                    string body = reader.GetString (2);
                    reader.Close ();
                    this.ParseProcedureBody (parametersTable, body, routine, nameToRestrict);
                }
            }
    ...
    (
        MySqlSchemaCollection parametersTable, string body,
    ...
    )
    {
        ...
        while (token != ")")
        {
            ...
            // now parse data type
            token = this.ParseDataType (parmRow, tokenizer);
        ...
        if (string.Compare (token, "RETURNS", StringComparison.OrdinalIgnoreCase) == 0)
        {
            ...
            this.ParseDataType (parameterRow, tokenizer);
        }
    }
    ...
    {
        StringBuilder dtd = new StringBuilder (StringUtility.ToUpperInvariant (tokenizer.NextToken ()));
               ...
               string.Compare (token, "return", StringComparison.OrdinalIgnoreCase) != 0)
        {
            if (string.Compare (token, "CHARACTER", StringComparison.OrdinalIgnoreCase) == 0 ||
                string.Compare (token, "BINARY",    StringComparison.OrdinalIgnoreCase) == 0) { } // we don't need to do anything with this
        ...
        if (row ["CHARACTER_MAXIMUM_LENGTH"] != null)
        {
            ...
            row ["CHARACTER_OCTET_LENGTH"] =
                CharSetMap.GetMaxLength ((string) row ["CHARACTER_SET_NAME"], this.connection) *
    ...
    private static void ParseDataTypeSize (MySqlSchemaRow row, string size)
    {
        ...
        {
            row ["CHARACTER_MAXIMUM_LENGTH"] = int.Parse (parts [0], CultureInfo.InvariantCulture);
            // will set octet length in a minute
        }
        else
        {
            row ["NUMERIC_PRECISION"] = int.Parse (parts [0], CultureInfo.InvariantCulture);

            if (parts.Length == 2)
                row ["NUMERIC_SCALE"] = int.Parse (parts [1], CultureInfo.InvariantCulture);
        }
    }

...
public abstract class MySqlBaseConnectionStringBuilder : DbConnectionStringBuilder
{
    ...
    static MySqlBaseConnectionStringBuilder ()
    {
        ...
        Options.Add (
            new MySqlConnectionStringOption (
                "protocol", "connection protocol,connectionprotocol", typeof (MySqlConnectionProtocol), MySqlConnectionProtocol.Sockets, false,
        ...
        Options.Add (
            new MySqlConnectionStringOption (
                "certificatestorelocation", "certificate store location", typeof (MySqlCertificateStoreLocation), MySqlCertificateStoreLocation.None, false
            )
        );
        ...
        Options.Add (
            new MySqlConnectionStringOption (
                "sslmode", "ssl mode,ssl-mode", typeof (MySqlSslMode), MySqlSslMode.Preferred, false,
                (SetterDelegate) ((msb, sender, value) =>
                {
        ...
        Options.Add (
            new MySqlConnectionStringOption (
                "sslca", "ssl-ca", typeof (string), null, false,
        ...
        Options.Add (
            new MySqlConnectionStringOption (
                "tlsversion", "tls-version,tls version", typeof (string), null, false,
                (SetterDelegate) ((msb, sender, value) =>
                {
    ...
    private static string TlsValidation (object value)
    {
        ...
        strProtocols = value.ToString ().Equals ("Tls13",      StringComparison.OrdinalIgnoreCase)
                       || value.ToString ().Equals ("Tlsv1.3", StringComparison.OrdinalIgnoreCase)
            ? "Tls13"
            : strProtocols;
...
#region Server Properties
    ...
    {
        get => this.values ["database"] as string;
    ...
    {
        get => (MySqlConnectionProtocol) this.values ["protocol"];
...
#region Authentication Properties
    ...
    {
        get => (string) this.values ["user id"];
    ...
    {
        get => (string) this.values ["password"];
    ...
    {
        get => (string) this.values ["password2"];
        set => this.SetValue ("password2", value);
    }
    ...
    {
        get => (string) this.values ["password3"];
        set => this.SetValue ("password3", value);
    }
    ...
    [Description (
        "Certificate file in PKCS#12 format (.pfx) or path to a local file that " +
        "contains a list of trusted TLS/SSL CAs (.pem)."
    )]
    public string CertificateFile
    {
        get => (string) this.values ["certificatefile"];
    ...
    {
        get => (string) this.values ["certificatepassword"];
    ...
    {
        get => (MySqlCertificateStoreLocation) this.values ["certificatestorelocation"];
    ...
    {
        get => (string) this.values ["certificatethumbprint"];
    ...
    {
        get => (MySqlSslMode) this.values ["sslmode"];
    ...
    public string SslCa
    {
        get => this.CertificateFile;
        set => this.CertificateFile = value;
    }
    ...
    {
        get => (string) this.values ["tlsversion"];
    ...
    {
        get => (string) this.values ["sslcert"];
...
#region Other Properties
    ...
    [Description (
        "For TCP connections, the idle connection time (in seconds) before the first keepalive packet is sent." +
        "A value of 0 indicates that keepalive is not used."
    )]
    [DefaultValue (0)]
    public uint Keepalive
    {
        get => (uint) this.values ["keepalive"];
...
#region Language and Character Set Properties
    ...
    {
        get => (string) this.values ["characterset"];
    ...
    internal void AnalyzeConnectionString (string connectionString, bool isXProtocol, bool isDefaultPort = true)
    {
        ...
        foreach (string query in queries)
        {
            ...
            string keyword = keyValue [0].ToLowerInvariant ().Trim ();
            string value   = query.Contains (",") ? query.Replace (keyword, "") : keyValue [1].ToLowerInvariant ();

            MySqlConnectionStringOption option = Options.Options.Where (o => o.Keyword == keyword || (o.Synonyms != null && o.Synonyms.Contains (keyword)))
                                                        .FirstOrDefault ();
            ...
            if (isDnsSrv)
            {
                ...
                if (option.Keyword == "server" &&
                    ((value.Contains ("address") && value.Split (new string [] {","}, StringSplitOptions.RemoveEmptyEntries).Length > 2) ||
                     value.Contains (",")))
